...
`claude-usage-bar` shows your Claude Code **5-hour** and **weekly** usage limits in the macOS menu bar. This doc explains the mechanism, why it's built this way, and how to verify each link yourself.
...
`claude-usage-bar hook` is registered as your statusline command (`~/.claude/settings.json`):
...
- **Native app** (`native/ClaudeUsageMenuBar.swift`, a single Swift file, ~200 lines, no runtime dependencies): an `NSStatusItem` that renders the two percentages as real two-line text, refreshes on a 60s timer and when you open the menu, and adapts its color to a light/dark menu bar. Runs via a LaunchAgent. `claude-usage-menubar --once` prints what it would show and exits.
- **SwiftBar plugin** (`plugins/claude-usage.1h.sh`): calls `claude-usage-bar render`; refreshes hourly, on click, and via a menu item.

## Freshness model

The snapshot advances only when Claude Code renders its statusline (an event-driven, roughly-per-turn thing while you work). When no session is active the reader shows the last written value and marks it stale after 3h (`CLAUDE_USAGE_BAR_STALE_MINUTES`). Two cases make the last value lag until the next local render: a window **resetting** while you're away, and usage spent on **another device**. Both self-correct on the next render.
...
```bash
claude-usage-bar path                  # where the snapshot lives
cat "$(claude-usage-bar path)"          # the raw snapshot
claude-usage-bar doctor                 # is the hook wired? a reader installed? snapshot fresh?
claude-usage-menubar --once             # what the native menu bar would show, printed
```
