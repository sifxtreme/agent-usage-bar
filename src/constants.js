 ...
 *
 * Override with AGENT_USAGE_BAR_SNAPSHOT. Defaults under the Claude config
...
export function snapshotPath(env = process.env, homeDir = os.homedir()) {
  const override = env.AGENT_USAGE_BAR_SNAPSHOT?.trim();
  if (override) {
 ...
 * Override with AGENT_USAGE_BAR_STALE_MINUTES.
 */
export function staleAfterMs(env = process.env) {
  const raw = Number(env.AGENT_USAGE_BAR_STALE_MINUTES);
...
export function menuStyle(env = process.env) {
  return (env.AGENT_USAGE_BAR_STYLE?.trim() || 'numbers').toLowerCase();
}
export function barSegments(env = process.env) {
  const n = Number(env.AGENT_USAGE_BAR_SEGMENTS);
